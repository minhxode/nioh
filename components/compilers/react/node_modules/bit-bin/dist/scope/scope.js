...
class Scope {
  ...
  getDependencyGraph() {
    ...
    return (0, _bluebird().coroutine)(function* () {
      if (!_this._dependencyGraph) {
        _this._dependencyGraph = yield _scopeGraph().default.loadAllVersions(_this);
      }

      return _this._dependencyGraph;
    })();
  } // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  migrate(verbose) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const scopeVersion = _this2.scopeJson.get('version') || '0.10.9';
      ...
      const rawObjects = yield _this2.objects.listRawObjects();
      ...
      _this2.objects.addMany(resultObjects.newObjects); // Remove old objects


      _this2.objects.removeManyObjects(resultObjects.refsToRemove); // Persists new / remove objects


      const validateBeforePersist = false;
      yield _this2.objects.persist(validateBeforePersist); // Update the scope version

      _this2.scopeJson.set('version', _constants().BIT_VERSION);

      _logger().default.debugAndAddBreadCrumb('scope.migrate', `updating scope version to version ${_constants().BIT_VERSION}`);

      yield _this2.scopeJson.write(_this2.getPath());
  ...
  list() {
    ...
    return (0, _bluebird().coroutine)(function* () {
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
      return _this3.objects.listComponents();
    })();
  }
  ...
  listIncludesSymlinks() {
    ...
    return (0, _bluebird().coroutine)(function* () {
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
      return _this4.objects.listComponentsIncludeSymlinks();
    })();
  }
  ...
  listLocal() {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const listResults = yield _this5.list();
      return listResults.filter(result => !result.scope || result.scope === _this5.name);
    })();
  }
  ...
  latestVersions(componentIds, throwOnFailure = true) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const components = yield _this6.sources.getMany(componentIds);
  ...
  injectNodePathIfNeeded(consumer, components) {
    ...
    const isNodePathNeeded = nodePathDirDist && components.some(component => (component.dependencies.isCustomResolvedUsed() || component.devDependencies.isCustomResolvedUsed() || component.compilerDependencies.isCustomResolvedUsed() || component.testerDependencies.isCustomResolvedUsed()) && component.componentMap && component.componentMap.origin === _constants().COMPONENT_ORIGINS.AUTHORED && !component.dists.isEmpty());
  ...
  testMultiple({
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      _this8.injectNodePathIfNeeded(consumer, components);
  ...
  writeManyComponentsToModel(componentsObjects, persist = true) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      yield (0, _pMapSeries().default)(componentsObjects, componentObjects => componentObjects.toObjectsAsync(_this9.objects).then(objects => _this9.sources.merge(objects)));
      return persist ? _this9.objects.persist() : Promise.resolve();
    })();
  }
  ...
  removeMany(bitIds, force, removeSameOrigin = false, consumer) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const removeComponents = new (_removeModelComponents().default)(_this11, bitIds, force, removeSameOrigin, consumer);
  ...
  findDependentBits(bitIds, returnResultsWithVersion = false) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const allComponents = yield _this12.list();
      const allComponentVersions = yield Promise.all(allComponents.map(
      /*#__PURE__*/
      function () {
        var _ref4 = (0, _bluebird().coroutine)(function* (component) {
          const loadedVersions = yield Promise.all(Object.keys(component.versions).map(
          /*#__PURE__*/
          function () {
            var _ref5 = (0, _bluebird().coroutine)(function* (version) {
              const componentVersion = yield component.loadVersion(version, _this12.objects);
  ...
  filterFoundAndMissingComponents(bitIds) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const resultP = bitIds.map(
      /*#__PURE__*/
      function () {
        var _ref6 = (0, _bluebird().coroutine)(function* (id) {
          const component = yield _this13.getModelComponentIfExist(id);
  ...
  loadLocalComponents(ids) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const componentsObjects = yield _this14.sources.getMany(ids);
  ...
  getModelComponent(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const component = yield _this15.getModelComponentIfExist(id);
  ...
  getModelComponentIgnoreScope(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      if (!id.scope) {
        ...
        const components = yield _this16.list();
  ...
  getConsumerComponent(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const modelComponent = yield _this17.getModelComponent(id); // $FlowFixMe version must be set
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

      const componentVersion = modelComponent.toComponentVersion(id.version); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

      return componentVersion.toConsumer(_this17.objects);
    })();
  }
  ...
  getVersionInstance(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const component = yield _this18.getModelComponent(id); // $FlowFixMe id.version is not null, was checked above
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

      return component.loadVersion(id.version, _this18.objects);
    })();
  }
  ...
  getComponentsAndVersions(ids) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const componentsObjects = yield _this19.sources.getMany(ids);
      const componentsAndVersionsP = componentsObjects.map(
      /*#__PURE__*/
      function () {
        var _ref7 = (0, _bluebird().coroutine)(function* (componentObjects) {
          ...
          const version = yield component.loadVersion(versionStr, _this19.objects);
          return {
            component,
            version,
            versionStr
  ...
  getComponentsAndAllLocalUnexportedVersions(ids) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const componentsObjects = yield _this20.sources.getMany(ids);
      const componentsAndVersionsP = componentsObjects.map(
      /*#__PURE__*/
      function () {
        var _ref8 = (0, _bluebird().coroutine)(function* (componentObjects) {
          ...
          return Promise.all(localVersions.map(
          /*#__PURE__*/
          function () {
            var _ref9 = (0, _bluebird().coroutine)(function* (versionStr) {
              const version = yield component.loadVersion(versionStr, _this20.objects);
              return {
                component,
                version,
                versionStr
  ...
  findDirectDependentComponents(componentsPool, potentialDependencies) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const componentsVersions = yield _this21.loadLocalComponents(componentsPool);
      const dependentsP = componentsVersions.map(
      /*#__PURE__*/
      function () {
        var _ref10 = (0, _bluebird().coroutine)(function* (componentVersion) {
          const component = yield componentVersion.getVersion(_this21.objects);
  ...
  runComponentSpecs({
    ...
    return (0, _bluebird().coroutine)(function* () {
      if (!bitId.isLocal(_this22.name)) {
        throw new (_generalError().default)('cannot run specs on remote component');
      }

      const component = yield _this22.getConsumerComponent(bitId);
  ...
  build({
    ...
    return (0, _bluebird().coroutine)(function* () {
      if (!bitId.isLocal(_this23.name)) {
        throw new (_generalError().default)('cannot run build on remote component');
      }

      const component = yield _this23.getConsumerComponent(bitId);
  ...
  loadModelComponentByIdStr(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      return _this24.objects.load(ref);
    })();
  }
  /**
  ...
  isIdHasScope(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const component = yield _this25.loadModelComponentByIdStr(id);
  ...
  getParsedId(id) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const idHasScope = yield _this26.isIdHasScope(id);
      return _bitId().BitId.parse(id, idHasScope);
    })();
  }
  ...
  static load(absPath) {
    return (0, _bluebird().coroutine)(function* () {
      ...
      const scopeJson = yield _scopeJson().ScopeJson.loadFromFile((0, _scopeJson().getPath)(scopePath));
