...
function _extensionFile() {
  const data = _interopRequireDefault(require("../../extensions/extension-file"));
...
class Version extends _objects().BitObject {
  ...
  constructor(props) {
    ...
    this.compilerDependencies = new (_dependencies().Dependencies)(props.compilerDependencies);
    this.testerDependencies = new (_dependencies().Dependencies)(props.testerDependencies);
    ...
    this.flattenedCompilerDependencies = props.flattenedCompilerDependencies || new (_bitId().BitIds)();
    this.flattenedTesterDependencies = props.flattenedTesterDependencies || new (_bitId().BitIds)();
  ...
  id() {
    ...
    const filterFunction = (val, key) => {
      if (key === 'devDependencies' || key === 'compilerDependencies' || key === 'testerDependencies' || key === 'devPackageDependencies' || key === 'peerPackageDependencies' || key === 'overrides') {
  ...
  getAllFlattenedDependencies() {
    return _bitId().BitIds.fromArray([...this.flattenedDependencies, ...this.flattenedDevDependencies, ...this.flattenedCompilerDependencies, ...this.flattenedTesterDependencies]);
  }
  ...
  getAllDependenciesIds() {
    ...
    return _bitId().BitIds.fromArray(allDependencies.map(dependency => dependency.id));
  }
  ...
  toObject() {
    ...
    const _convertEnvToObject = env => {
      ...
      if (env.files && !_ramda().default.isEmpty(env.files)) {
        result.files = env.files.map(_extensionFile().default.fromModelObjectToObject);
      }
...
function parseEnv(env) {
  ...
  return {
    name: env.name,
    config: env.config,
    files: env.files ? env.files.map(_extensionFile().default.fromObjectToModelObject) : []
  };
}
