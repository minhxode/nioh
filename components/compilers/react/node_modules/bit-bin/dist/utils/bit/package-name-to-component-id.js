...
exports.packageNameToComponentId = packageNameToComponentId;
...
function packageNameToComponentId(consumer, packageName, bindingPrefix) {
  const componentName = getComponentName(packageName, bindingPrefix);
  const nameSplit = componentName.split(_constants().NODE_PATH_COMPONENT_SEPARATOR);
  ...
  function _handleNoDotCase() {
    // see constrain #1 above.
    if (allBitIds.searchWithoutVersion(idWithoutScope)) {
      return idWithoutScope;
    }

    throw new (_generalError().default)(`packageNameToComponentId unable to determine the id of ${componentName}, it has no dot and is not exists on .bitmap`);
  }

  function _handleOneDotCase() {
  ...
  function _handleTwoOrMoreDotsCase() {
    ...
    if (allBitIds.searchWithoutVersion(idWithScopeWithoutDot)) return idWithScopeWithoutDot; // it's not on .bitmap, we can't determine whether it's option #2 or option #3.
  ...
  function _idConsiderDefaultScope() {
    ...
    if (defaultScope && componentName.startsWith(`${defaultScope}.`)) {
      ...
      if (idWithRemovedScope) return idWithRemovedScope; // otherwise, the component is not in .bitmap, continue with other strategies.
    }
...
function getComponentName(packageName, bindingPrefix) {
...
function createBitIdAssumeNoScope(nameSplit) {
...
function createBitIdAssumeScopeHasDot(nameSplit) {
...
function createBitIdAssumeScopeDoesNotHaveDot(nameSplit) {
  const scope = _ramda().default.head(nameSplit);

  const name = _ramda().default.tail(nameSplit).join('/');
...
function createBitIdAssumeDefaultScope(defaultScope, nameSplit) {
  return defaultScope.includes('.') ? createBitIdAssumeScopeHasDot(nameSplit) : createBitIdAssumeScopeDoesNotHaveDot(nameSplit);
}